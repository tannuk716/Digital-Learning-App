...
class HoneyCountActivity : AppCompatActivity() {

    private lateinit var tvJars: TextView
    private lateinit var btnOpt1: Button
    private lateinit var btnOpt2: Button
    private lateinit var btnOpt3: Button

    private val puzzles = listOf(
        Pair("🍯 🍯 🍯", listOf("2", "3", "4") to 1),
        Pair("🍯 🍯 🍯 🍯 🍯", listOf("5", "6", "4") to 0),
        Pair("🍯 🍯", listOf("1", "2", "3") to 1),
        Pair("🍯 🍯 🍯 🍯 🍯 🍯 🍯", listOf("6", "8", "7") to 2)
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_honey_count)

        tvJars = findViewById(R.id.tvJars)
    ...
    private fun loadPuzzle() {
        if (currentIndex >= puzzles.size) {
            Toast.makeText(this, "Great job! Bear is full!", Toast.LENGTH_LONG).show()
            finish()
            return
        }

        val data = puzzles[currentIndex]
        tvJars.text = data.first
